...
class AdultDataset(data: DataFrame) extends Dataset(data) with ReflectionCompanion {

  def companion = companionOf[AdultDataset]

  if (!data.schema.equals(companion.getSchema())) throw new IllegalArgumentException("The given DataFrame does not respect the defined schema.")
  preprocess()
  ...
  override def preprocess(): DataFrame = {
    val funcs = Seq(
      ...
      Preprocessor.binning(_: DataFrame, "hours-per-week", Array(0, 25, 40, 60, 100)),
